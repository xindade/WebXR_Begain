...
> ★ **2026-09-24（第二十三 / 二十四修）：本方案的服务器授权校验已降级为可选** —— 正式包默认**关闭**
> （`LICENSE_ENABLED = hasFlag('license')`），取代它的是「服务器清单」下发配置。
> 详见 `docs/tech/09-服务器清单与旧配置清理.md`；下文 §1~§12 作为「真要做硬授权」时的完整设计**原样保留**。
...
## 附 · 2026-09-24：授权校验默认关闭 + 配置改由「服务器清单」（第二十三 / 二十四修）

### 一句话

**需求方 2026-09-24 拍板：不再做加密狗式的服务器授权校验。**
本方案的 `licenseGate()` 已降级为**可选**（`LICENSE_ENABLED = hasFlag('license')`，正式包默认 **关闭**），
取代它的是「**每次启动从服务器拉一份配置清单**」—— 见 `docs/tech/09-服务器清单与旧配置清理.md`。

### 关闭后各处的实际行为

| 位置 | 行为 |
|---|---|
| `tools/cast-pc/main.js` `licenseGate()` | **一律放行**（不再因「未激活 / 已到期」拒绝头显） |
| PC 接收端界面 | 不显示授权面板、不联网续期、不做激活 |
| `/api/info`、`/api/license/current` | 回报 `license.mode='off'`、`license.off=true` |
| 头显 `LicenseVerify` / `fetchExeLicense()` | 读到 `off` 即**跳过** license 校验 |
| 恢复旧行为 | 启动加 `--license`（排障 / 回归用） |

### 与「内容门禁」的分工（改动后的三段判据）

| 段 | 判据 | 现状 |
|---|---|---|
| ① `licenseGate()` | 内容授权门禁（本方案 §4） | **默认关闭**（`--license` 恢复） |
| ② 凭据三选一 | 签名 `dev+ts+sig` / 放行条 `dev+exp+voucher` / 仅 `device` | 不变 |
| ③ `ROUND.armed` | 「这一局允不允许」（等平台点开始游戏） | 不变（第二十修） |

**新增**的「服务器清单」**不参与**上述三段判定 —— 它是**配置的取用口径**，不是放行判据。
两者唯一的交点：`POST /api/config/dump` 在凭据校验后要先 `await manifestEnsure()`（清单不到位就 `allow:false`）。

### 保留（不可顺手删）

- 本方案 §1~§12 的完整设计（三把钥匙 / PoP 绑机 / 离线兜底 0 天 / 服务器 P1 实现 / 六套自测）**全部保留**：
  真要做硬授权时照它做，且 `tools/cast-server/` 的 license 接口**仍在运行**（只是没人调）。
- ⚠ **不要**因为「现在默认关闭」就删掉 `licenseGate()` 与 `/api/license/*` —— 商业上随时可能重新要求硬授权，
  重建成本远高于保留成本。

### 验证（打包版实跑，2026-09-24）

- `/api/info` → `license: { ok:true, mode:"off", off:true, why:"授权验证已关闭（第二十三修）", daysLeft:null }`。
- 启动日志：`[cast-pc] 授权验证：**已关闭**（第二十三修 · 默认）—— 不校验授权、不联网续期；加 --license 可恢复旧行为`。
