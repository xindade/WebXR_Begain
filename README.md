...
src/
  ...
  ui/
    ...
    pagelog.js         最高优先级页面日志：浏览器最左侧 1/4 面板（经典脚本，不依赖 three，
                       可捕获黑屏类致命错误；退出 VR 自动暂停滚动防丢失）
  content/
...
**页面日志（调试最高优先）：** 浏览器页面**最左侧 1/4 区域**常驻一个日志面板（`src/ui/pagelog.js`），
捕获 `console.*` 与全局未处理异常 / Promise 拒绝（含 three 加载失败、模块语法错误这类"黑屏"元凶）。
它以**经典脚本**在 `main.js` 之前加载，因此即便 three 加载失败也能显示报错。
退出 VR 回到预览界面时，面板**自动暂停滚动**（顶部徽标变黄显示"⏸ 已暂停滚动"），防止最新报错被新日志顶掉；点徽标可恢复滚动。
此面板在头显沉浸式模式下不可见（DOM 被隐藏），但退出 VR 后立即可读。
