...
// ============================================================
// ============================================================
// 主控门禁（《打包和平台对接》第 3 条）—— **编译时常量**
//   true  = 正式包：APK 必须先拿到 PC 主控端的「允许运行」才进游戏，
//           游戏页面（头显浏览器）再独立复验一次（见 src/main.js 的 gateQueryMaster）。
//   false = 调试包：直接放行，现场自测不必改网址。
// 运行时旁路（优先级高于本常量）：网址加 ?gate=0 → 放行；?gate=1 → 强制打开做对照。
// ⚠ 打包脚本若要区分正式/调试包，改的就是这一个值。
// ============================================================
export const MASTER_GATE = true;

// ============================================================
// 正式版界面裁剪 + **功能裁剪**（2026-09-23 需求）—— **编译时常量**
//   true  = 正式包：不创建「暂停 / 继续 / 设置 / 日志 / 导出性能记录」控制条与桌面「开始游戏」
//           按钮，隐藏右侧「关卡快捷」选关面板、枪支模式、操作提示；并且**功能本身不存在**：
//           暂停不再触发（focusPause 直接返回、失焦/可见性监听不注册、A/B 菜单键分支不走）、
//           性能监视器不构造（不采样）、页面日志 disable()（不建 DOM）、设置对话框无入口。
//   false = 调试包：全部保留（开发期照旧）。
// 运行时旁路（优先级高于本常量）：网址加 ?devui=1 → 强制恢复全部调试入口（现场排障，不必重打包）。
// ⚠ 打包脚本若要区分正式/调试包，改的就是这一个值（与上面的 MASTER_GATE 并列）。
// ============================================================
export const RELEASE_UI = true;
