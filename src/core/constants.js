...
export const FLIP = {
  ...
  COUNTDOWN: 180,           // 安全解谜期总时长(秒)：归零或解出均进入下一关、无抽卡
  COLS: 3, ROWS: 3,        // 3×3 = 9 格
  CELL: 1,                 // 每格 1.0m
  BASE: { x: 0, y: 4, z: 0 },   // 九宫格底边中心（世界坐标，竖直墙面、朝向玩家）
  BALLOON_R: 0.5,          // 气球碰撞半径（命中判定用）
  BALLOON_OFFSET_Z: 0.35,  // 气球相对格中心沿 +z 凸向玩家的偏移（白=前/黑=后同此值）
  WALL_WIDTH: 3,           // 整墙 3m 宽
  WALL_HEIGHT: 3,          // 整墙 3m 高（y=4~7）
  FLIP_DUR: 0.5,           // 单格翻转动画时长 s
  VICTORY_DUR: 1.0,        // 胜利闪烁后消失时长 s
  HIT_R: 0.5,              // 子弹命中判定半径（≈气球半径）
