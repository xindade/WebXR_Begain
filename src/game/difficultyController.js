...
export class DifficultyController {
  ...
  update(dt) {
    ...
    // 玩家越强、血越多、击杀越快 → 难度越高
    // 场上威胁高时略微收敛，避免雪崩
    const powerNorm = Math.min(1, power / 200);          // 归一化战力（初始~20 → 0.1，升满~200 → 1.0）
    const killNorm  = Math.min(1, this.killRate / 1.5);   // 归一化击杀率（1.5 kills/s = 满分）
  ...
  getPlan() {
    ...
    return {
      ...
      cooldown: Math.max(0.3, 1.0 - d * 0.5),  // 1.0s → 0.5s（缩短出怪间隔，加快补充）
      dist: 12,
      spread: 8,
      // phase 不再硬编码：由 waves._phase() 按时间分阶段控制出怪方向
